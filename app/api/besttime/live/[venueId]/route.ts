...
const getErrorMessage = (error: unknown, credentials: BestTimeCredentials) => {
  const message = error instanceof Error ? error.message : 'Unable to load live venue data'
  ...
  return typeof redacted === 'string' ? redacted : 'Unable to load live venue data'
}
...
export async function GET(request: NextRequest, context: RouteParams) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      venueId,
      status: venue.liveStatus,
      liveBusyness: venue.liveBusyness,
      message: venue.liveBusyness === undefined
        ? 'Live busyness is not available for this venue.'
        : 'Live busyness is available for this venue.'
    })
  } catch (error) {
    return NextResponse.json(
