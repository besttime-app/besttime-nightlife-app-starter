...
- When a visitor supplies browser-stored private and public BestTime API keys, live BestTime result detail links must use a client-side, noindex route that reads local storage and calls the starter API proxy with headers. Server-rendered `/venues/*` pages are reserved for fixture/known venue SEO pages because they cannot access browser local storage.
...
- When `BESTTIME_PUBLIC_API_KEY` or a browser-stored public key is available, live venue detail pages should hydrate weekly heatmaps from the split `forecasts/week/raw2` response instead of cloning the selected venue-filter day across all seven rows.
